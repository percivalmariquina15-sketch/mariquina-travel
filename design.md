...
- **Mobile menu (below `xl`):** hamburger button (40×40, rounded 8px, ink/white per scroll state, `aria-expanded`) toggles a white panel below the bar (`border-t #E2E8F0`): stacked links (44px rows, 14px Semi-bold, active = amber, follows scroll-spy), divider, phone link, full-width "Message Us" button. Panel closes on link click, outside click, and Escape. Link taps close the panel first, then smooth-scroll to the target (deferred ~60ms so the panel height is removed first — prevents landing off-target). Desktop nav hides below `xl`.
...
- **Hero:** `min-h-svh` (not `min-h-screen`) so the full-height section uses the small viewport height — no URL-bar jump on mobile browsers. Headline 36px mobile → 52px `sm` → 56px `lg`, wraps naturally. Scroll-hint badge sits `bottom-24px`; content padded `pt-36 pb-20`.
